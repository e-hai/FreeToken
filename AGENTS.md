...
## AI 图像生成外挂规范 (Image Generation Workflow)

系统已为 Agent 挂载 AI 图像生成工具（支持 Flux / Google Imagen 3 智能双引擎容灾极速高清出图）。
...
2. **MCP 工具调用**：
   - 工具名：`generate_image`
   - 参数：`prompt` (画面描述), `size` (尺寸如 `1024x1024`, `1280x720`, `720x1280`), `output_path` (保存路径)

### 容灾策略：
- 默认采用 **Flux.1 免 Key 开源旗舰** 引擎快速出图；
- 若遇网络波动或 Flux 服务异常，网关将**自动无缝容灾切换至 Google Imagen 3** 扩散大模型完成渲染。
