...
cleanup() {
    ...
    lsof -ti :8000 -ti :8001 2>/dev/null | xargs kill -9 2>/dev/null || true
...
# ==================== 5. 启动网关服务 (后台) ====================
echo "=========================================================================="
echo "⚡ Free Token 双轨聚合网关 (Harness API & Codex)"
echo "=========================================================================="
...
echo "🚀 正在启动 Free Token 双轨网关 (端口 8000 & 8001)..."
...
# ==================== 6. 可选启动 DeepSeek-Harness Web ====================
if [ "$WITH_HARNESS" -eq 1 ]; then
    source "$DIR/harness/harness_env.sh"
    echo ""
    echo "🚀 正在启动 DeepSeek-Harness Web (端口 3080)..."
...
if [ -n "$HARNESS_PID" ]; then
    ...
    echo "   ℹ  DeepSeek-Harness 未启动。需要时请运行: ./start_gateway.sh --with-harness"
    echo "      或单独执行: source harness/harness_env.sh && npx @deepseek-ai/dsh web"
fi
echo "=========================================================================="
echo "💡 按 Ctrl+C 可停止服务"
echo "=========================================================================="
